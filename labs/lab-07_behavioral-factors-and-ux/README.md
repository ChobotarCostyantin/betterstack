...
Зафіксовано 5 ключових мікроконверсій для проєкту **betterstack.tech**, що дозволяють оцінити якість органічного трафіку до моменту здійснення макро-цілі (реєстрації або переходу на сайт вендора).

| Мікроконверсія | Event name | Де тригериться | Навіщо для SEO | Поточне значення | Ціль на 30 днів |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Скрол 75% статті** | `scroll_75` | `/article/*` | Підтверджує якість контенту та повну відповідність інтенту користувача | 41% сесій | 55% сесій |
| **Додавання софту у стек** | `add_to_stack` | `/article/*` | Сигнал високої користі сторінки та залученості в екосистему інструменту | ~6% сесій | 10% сесій |
| **Клік на порівняння** | `click_comparison` | `/article/*` | Свідчить про зацікавленість у глибшому аналізі та задоволення інформаційного запиту | ~5% сесій | 15% сесій |
| **Зміна параметрів порівняння** | `change_comparison` | `/comparison` | Демонструє найвищий рівень взаємодії та цінність інструментарію сайту для SEO-ранжування | 7% сесій | 12% сесій |
| **Фільтрація в каталозі** | `filter_category` | `/catalog` | Підтверджує релевантність структури категорій та зручність пошуку специфічних рішень | ~18% сесій | 25% сесій |
---
