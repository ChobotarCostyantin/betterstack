...
| :--- | :--- | :--- |
| **Submitted and indexed** | Сторінка була успішно додана в карту сайту (Sitemap), просканована роботом і тепер відображається в результатах пошуку. | Все налаштовано правильно: контент якісний, технічних перешкод немає. |
| **Crawled - currently not indexed** | Googlebot відвідав сторінку та прочитав її вміст, але вирішив поки що не додавати її до пошукової видачі. | Низька якість контенту, сторінка є дублікатом іншої, або Google вважає, що вона не несе цінності для користувачів. |
| **Discovered - currently not indexed** | Google знає про існування цього URL (знайшов посилання на нього або в Sitemap), але ще навіть не намагався його просканувати, щоб не витрачати ресурси (crawl budget). | Велика кількість нових сторінок на сайті одночасно, погана внутрішня перелінковка або низький авторитет сайту. |
| **Excluded by noindex tag** | Google знайшов сторінку, але не додав її в індекс, бо зустрів пряму заборону від розробника. | На сторінці присутній мета-тег `<meta name="robots" content="noindex">`. Часто забувають зняти після розробки або використовують для службових сторінок (кошик, адмінка). |
| **Blocked by robots.txt** | Пошуковий робот не зміг навіть зайти на сторінку, оскільки доступ до цієї директорії закритий у конфігураційному файлі сайту. | У файлі `robots.txt` прописано правило `Disallow` для цього шляху. Це часто робиться для папок з логами, скриптами або API-ендпоінтами. |
| **Redirect error** | Робот намагався перейти за посиланням, але виникла технічна помилка під час перенаправлення. | "Циклічна" переадресація (A веде на B, а B — на A), занадто довгий ланцюжок редиректів або порожній URL призначення. |
| **404 Not Found** | Сторінка, яка раніше була в індексі або на яку веде посилання, фізично більше не існує за цією адресою. | Видалення сторінки софту без налаштування 301 редиректу на аналог, або зміна структури URL (наприклад, зміна slug) без автоматичного перенаправлення. |
| **Soft 404** | Сервер каже, що сторінка "є" (код 200), але її вміст виглядає як помилка або вона майже порожня. | Сторінка результатів пошуку, яка не знайшла жодного софту, або сторінка категорії без товарів, яка замість коду 404 віддає стандартний 200 OK. |
...
**Чому?**
BetterStack — це технічний ресурс (guides, documentation, community posts). Технічні фахівці шукають відповіді на дуже конкретні, складні запити (наприклад, _"how to handle logs in kubernetes with vector"_).
...
### 3. Впровадження E-E-A-T у проєкт BetterStack

**E-E-A-T** (Experience, Expertise, Authoritativeness, Trustworthiness) — це критичний набір критеріїв якості Google. Для BetterStack, як технічного ресурсу про програмне забезпечення, ці сигнали є ключовими для ранжування в ніші "Software Reviews".

#### 3.1 — Сторінка "Про нас" `/about`

У проєкті реалізовано статичну сторінку `frontend/src/app/about/page.tsx`, яка слугує головним вузлом довіри.

- **Назва та опис:** BetterStack визначено як платформу для об'єктивного порівняння софту та інструментів розробки на основі вимірюваних метрик.
- **Редакційна політика:** Описано процес збору даних (benchmarking) та додавання факторів, що мінімізує суб'єктивність.
- **Соціальні мережі:** Додано посилання на офіційний репозиторій проєкту в GitHub.

#### 3.2 — Профілі авторів

Для забезпечення **Expertise** (Експертності) було оновлено сутність `User` у базі даних та створено систему профілів.

- **Дані в БД:** Кожен автор має заповнені поля `fullName` (наприклад, _Tesey_), `bio` (опис технічного бекграунду), `avatarUrl` та `githubUrl`.
- **Сторінка автора:** Реалізовано відображення профілю, де виводиться:
  - Аватар розробника (через GitHub-лінк).
  - Біографія: _"Senior Full-stack Developer, maintainer"_.
  - Посилання на зовнішній авторитетний профіль (**GitHub**).

#### 3.3 — Підпис автора на сторінці статті

На динамічних сторінках програмного забезпечення `/article/[slug]` додано блок авторства, який з'єднує контент із реальною особою.

- **Компонент AuthorBox:** Додано у верхній або нижній частині опису софту.
- **Елементи:**
  - Фото автора та ім'я, що є клікабельним і веде на його повний профіль.
  - **Bio:** Коротка фраза про спеціалізацію автора в темі (наприклад, _"Expert in .NET and Go ecosystems"_).
- **Зв'язок:** Дані підтягуються через `relations: ['author']` у `SoftwareQueryService`.

#### 3.4 — E-E-A-T чек-ліст BetterStack

##### **Experience (Досвід)**

- [x] **Особистий досвід:** Огляди (наприклад, для Go або VS Code) містять реальні приклади конфігурації та коду, що вказує на практичне використання інструменту автором.
- [x] **Візуальні докази:** У полі `screenshotUrls` додано реальні знімки інтерфейсу програм, що підтверджує "hand-on" досвід.

##### **Expertise (Експертиза)**

- [x] **Технічна точність:** Замість загальних фраз використовуються конкретні метрики: _Memory footprint (MB)_, _Indexing time (s)_.
- [x] **Компетентність автора:** Автор має публічну історію контриб'ютингу в open-source, що верифікується через GitHub.

##### **Authoritativeness (Авторитетність)**

- [x] **Нішева спеціалізація:** Сайт фокусується виключно на Developer Tools, що підвищує його Topical Authority.
- [x] **Зовнішні сигнали:** Посилання на GitHub-репозиторії софту та офіційні сайти розробників створюють правильний "сусідський" профіль у графі знань Google.

##### **Trustworthiness (Надійність)**

- [x] **Безпека:** Проєкт використовує HTTPS та JWT-авторизацію для захисту даних користувачів.
- [x] **Прозорість:** Наявність сторінки `/about` та чітких метрик дозволяє користувачу довіряти актуальності даних порівняння.
- [x] **Відсутність маніпуляцій:** Метрики "Higher is better" чітко розмежовані, що виключає двозначність при порівнянні.

---

**Висновок:** Впроваджені зміни в архітектуру BetterStack (зв'язок Software-User та метадані автора) створюють необхідний фундамент для BERT-алгоритму Google, щоб ідентифікувати ресурс як такий, що створений експертами для спільноти.
...
| SEO Score                          | 100    | 100     |
| Accessibility Score                | 91     | 96      |
| Best Practices Score               | 96     | 96      |
| **LCP** (Largest Contentful Paint) | 3,0 с  | 0,4 с   |
| **CLS** (Cumulative Layout Shift)  | 0      | 0.001   |
| **FCP** (First Contentful Paint)   | 1,1 с  | 0,2 с   |
