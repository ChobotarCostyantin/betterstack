...
#### 2.1 — Контроль налаштувань

| Налаштування | Було | Стало | Доказ |
|--------------|------|-------|-------|
| robots.txt   | Відсутній | Додано файл robots.txt з базовими правилами індексації | https://betterstack.tech/robots.txt |
| sitemap.xml  | Відсутній | Додано sitemap.xml та підключено до robots.txt | https://betterstack.tech/sitemap.xml |
| canonical    | Частково реалізований, некоректний для URL з параметрами | Виправлено canonical для сторінок з параметрами (search, category, pagination) | DevTools (перевірка `<link rel="canonical">`) |
| Канонічна версія домену (HTTPS, www/non-www) | Реалізовано | Без змін, працює коректно | Перевірка через browser / curl |

---

#### 2.2 — Виправлення знайдених помилок

На основі аудиту (розділ 1) виправлено технічні проблеми:

| № | Проблема | Вплив на SEO | Що зроблено | Де перевірено | Статус |
|---|----------|--------------|-------------|---------------|--------|
| 1 | Canonical вів на URL з параметром | Розмивання сигналів, ризик дублів | Замінено canonical на релевантні URL (з урахуванням search/category/pagination) | DevTools + повторний crawl | Done |
| 2 | Відсутній robots.txt | Неконтрольований crawling, ризик індексації службових сторінок | Додано robots.txt з базовими правилами | Browser (https://betterstack.tech/robots.txt) | Done |
| 3 | Відсутній sitemap.xml | Ускладнена індексація нових сторінок | Створено sitemap.xml та додано в robots.txt | Browser + GSC | Done |
| 4 | Генеричні alt атрибути зображень (типу "screenshot 1") | Втрата SEO сигналів для зображень, гірша доступність | Додано можливість задавати кастомні alt; передача в image компоненти | DevTools (перевірка `<img alt="">`) | Done |
| 5 | Відсутність structured data (Author, Article, Review) | Гірше розуміння контенту пошуковими системами, відсутність rich results | Додано schema.org для Organization, Article, Author та Review | Rich Results Test | Done |
| 6 | 401 помилки при навігації між сторінками | Погіршення crawlability, можливе випадіння сторінок з індексу | Логіку авторизації перенесено на сервер (Next.js), перевірка виконується при рендерингу | DevTools (Network) + повторна навігація | Done |
| 7 | Клієнтський селектор категорій (button-based) | Погана crawlability, відсутність лінків для пошукових ботів | Перероблено на серверний компонент з `<a>` (anchor links) | DevTools (HTML) + crawl | Done |
...
|-------------------------|-----------------|-----------|
| robots.txt              | URL + ручна перевірка правил | Доступний, правила застосовуються коректно |
| sitemap.xml             | URL + перевірка у GSC         | Sitemap доступний, сторінки читаються |
| Canonical на ключових шаблонах | DevTools / crawler       | Canonical відповідає реальним URL без помилок |
| Канонічна версія домену (HTTPS, www/non-www) | curl / browser check | Редіректи працюють коректно, визначена одна канонічна версія |
| 4xx/5xx та redirect chains | crawler                     | Критичних помилок не виявлено |
| Schema.org (1-2 ключові шаблони) | Rich Results Test      | Валідно, помилок не виявлено |
...
- **Crawling:** Googlebot сканує URL і завантажує вихідний код сторінки.
- **Rendering:** Виконання JS, підвантаження CSS та зображень (робот "бачить" сторінку як користувач).
- **Indexing:** Аналіз відрендереного контенту та додавання його в базу пошуковика.
- **Чому важливо:** Закрито від crawling → немає індексу. Помилка rendering (напр., важкий JS) → Google індексує порожню сторінку і не бачить контент.

2. Чому canonical є "сильною рекомендацією", а не абсолютною командою для Google?

- Google оцінює безліч сигналів одночасно (внутрішні посилання, sitemap, HTTPS).
- Якщо canonical суперечить іншим сигналам (напр., веде на 404 помилку або всі внутрішні лінки ведуть на інший URL) → алгоритм його проігнорує.
- Google залишає за собою право самостійно обрати релевантніший URL для результатів пошуку.

3. Які порогові значення для LCP, INP і CLS вважаються хорошими?

- **LCP** (швидкість відмальовування найбільшого елемента): ≤ 2.5 сек.
- **INP** (затримка реакції на дії користувача): ≤ 200 мс.
- **CLS** (несподівані зсуви макета): ≤ 0.1.

4. У чому різниця між 301, 302, 404, 410 і коли який код варто використовувати?

- **301 (Moved Permanently):** Переїзд назавжди. Передає ~100% link equity. (Використання: зміна URL, злиття дублів, перехід на HTTPS).
- **302 (Found / Temporarily):** Тимчасово. Не передає вагу одразу. (Використання: A/B тести, техроботи, тимчасові акції).
- **404 (Not Found):** Сторінку не знайдено, невідомо чи назавжди. (Використання: биті лінки. Якщо є зовнішні посилання → краще ставити 301).
- **410 (Gone):** Видалено назавжди. (Використання: товар чи послугу знято назавжди. Прискорює видалення з індексу порівняно з 404).

5. Чим відрізняються природні (natural) і аутріч-посилання (outreach links)?

- **Природні (Natural):** Здобуті органічно. Вебмайстер сам знайшов ваш контент і поставив лінк → найцінніший і найбезпечніший тип посилань.
- **Аутріч (Outreach):** Здобуті через активну комунікацію. Ви самі шукаєте сайти і домовляєтесь із власниками/редакторами про розміщення (guest posting, broken link building).
...
На основі проведеного аудиту **betterstack.tech**, пріоритетність завдань розподілена за принципом критичності впливу на індексацію та стабільність ресурсу:

1.  **Створення та коректне налаштування `robots.txt` та `sitemap.xml`**: Це першочерговий етап, оскільки без цих файлів пошукові роботи не мають інструкцій щодо пріоритетних сторінок та обмежень (наприклад, доступу до `/admin/`), що призводить до неефективного використання краулінгового бюджету.
2.  **Усунення статус-кодів 500 (на прикладі `/article/python`)**: Серверні помилки є критичним негативним сигналом для Google. Постійні помилки 5xx призводять до деіндексації сторінок та зниження загального авторитету домену.
3.  **Виправлення тегів `rel="canonical"` на сторінках пагінації**: Поточна ситуація, де всі сторінки каталогу ведуть на першу сторінку, змушує Google ігнорувати унікальний вміст інших сторінок. Кожна сторінка пагінації має бути самоканонічною.
4.  **Впровадження структурованих даних (Schema.org)**: Це необхідно для покращення CTR (клікабельності) у видачі. Для нашого проєкту критично важливо додати розмітку `SoftwareApplication` та `Review`, щоб виділити картки інструментів серед конкурентів.
5.  **Очищення посилального профілю (Disavow спам-доменів)**: Виявлені 23 спам-посилання створюють ризик накладання алгоритмічних санкцій. Офіційна відмова від цих зв’язків через Google Search Console є необхідною для подальшого безпечного просування.

---

#### 12. Запропонуйте план, як утримувати Core Web Vitals без регресій після кожного релізу.

Для забезпечення стабільно високих показників швидкості необхідно впровадити наступні процеси:

* **Lighthouse CI в пайплайні розробки**: Інтеграція автоматизованого тестування в процес CI/CD. Якщо новий код знижує показник Performance нижче встановленого порогу (наприклад, 90), збірка (build) автоматично відхиляється.
* **Встановлення Performance Budgets**: Жорстке обмеження розмірів активів. Наприклад, сумарна вага JavaScript не повинна перевищувати 200 КБ, а зображення мають автоматично конвертуватися в WebP/AVIF.
* **Використання Real User Metrics (RUM)**: Моніторинг даних реальних користувачів через розділ Core Web Vitals у Google Search Console для виявлення проблем, які не помітні в лабораторних тестах Lighthouse.
...
| Характеристика | Швидкий спам (Black-hat) | White-hat (Outreach/Partners) |
| :--- | :--- | :--- |
| **Короткостроковий ефект** | Можливе різке зростання за низькочастотними запитами. | Відсутність помітного ефекту в перші 1-3 місяці. |
| **Довгостроковий ефект** | **Високий ризик санкцій (SpamBrain).** Повна втрата позицій або деіндексація сайту. | Поступове зростання DR (Domain Rating) та довіри з боку пошукових систем. |
| **Наслідки для Betterstack** | Погіршення і так проблемного профілю, що призведе до неможливості подальшого SEO. | Побудова авторитету через якісні донори (як-от **StackVarta**), що забезпечує стабільний трафік. |
...
Для поточного стану проєкту **більший ефект дадуть технічні виправлення**.

**Обґрунтування:**
Наразі сайт має фундаментальні технічні дефекти: відсутність карти сайту, помилки сервера (500) та некоректне використання канонічних тегів. Зовнішня оптимізація (лінкбілдінг) за таких умов буде неефективною, оскільки:
1.  Пошукові роботи, переходячи за новими посиланнями, натикатимуться на помилки доступу.
2.  Вага, що передається новими посиланнями, буде "розмиватися" через некоректні канонічні посилання.
3.  Google не підвищить рейтинг сайту з технічними помилками, навіть за наявності якісних донорів.

Технічне виправлення "фундаменту" дозволить існуючим і майбутнім сторінкам почати ранжуватися належним чином, що є критично важливим для нового домену.
